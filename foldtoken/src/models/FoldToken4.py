...
class PiFold_Model(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.vq = HierCVQLayer(args.vq_space, hidden_dim, 32, args.levels, args.condition_layer)
    ...
    def forward(self, X_true, chain_encoding, edge_idx, batch_id, X_t, V=None, E=None, T_ts=None, temp=1.0, mode='train', return_predX=False, batch_id_extend=None, edge_idx_extend=None, virtual_frame_num=3, vqshortcut=False, level=8):
        ...
        h_V, vq_code, vq_loss = self.vq(h_V, temperature=temp, mode=mode, vqshortcut=vqshortcut, level=level)
        ...
        return X_pred, all_preds, vq_loss, vq_count, vq_code
    ...
    def sample(self, 
        ...
        X_pred, all_preds, vq_loss, vq_count, vq_code = self(X, chain_encoding,
                                    edge_idx, batch_id, 
                                    X_t, V, E, T_ts, 
                                    temp, mode, 
    ...
    def decoding(self, h_V, chain_encoding, batch_id=None, returnX=False):
        ...
        X_pred, all_preds = self.decoder_struct.infer_X(X_t, h_V,  batch_id, chain_encoding, 30, virtual_frame_num=virtual_frame_num)
