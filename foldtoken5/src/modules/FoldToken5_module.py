...
class GeoFeaturizer(nn.Module):
    ...
    @classmethod
    @torch.no_grad()
    def from_X_to_features(self, X, node_idx, edge_idx, batch_id, chain_encoding, virtual_frame_num=0, atom_embed=None, input_layer=False):
        ...
        if virtual_frame_num>0:
            V, E = self.get_interact_feats(T, node_idx, T_ts, X, edge_idx, batch_id, atom_embed=atom_embed, input_layer=input_layer) 
    ...
    @classmethod
    @torch.no_grad()
    def get_interact_feats(self, T, node_idx, T_ts, X, edge_idx, batch_id, num_rbf=16, atom_embed=None, input_layer=False):
        dtype = X.dtype
        device = X.device
        ...
        if input_layer:
            ...
            E_bias = rbf_func(E_bias[:,None,None].float(),16).view(num_E,-1)
...
class GeneralGNN_noGeo(nn.Module):
    def __init__(self, 
        ...
        self.__dict__.update(locals())
        self.edge_mlp = nn.Linear(2*num_hidden+16, num_hidden)
        self.attention = PiFoldAttn(attn_layer, num_hidden, num_hidden, num_hidden, dropout) 
        self.update_node = UpdateNode(num_hidden)
        self.update_edge = UpdateEdge(edge_layer, num_hidden, dropout)
        self.mask_token = nn.Embedding(2, num_hidden)
    ...
    def forward(self, h_V, node_idx, edge_idx, batch_id):
        node_idx = rbf_func(node_idx[edge_idx[0]]-node_idx[edge_idx[1]], 16)
        h_E = self.edge_mlp(torch.cat([h_V[edge_idx[0]], h_V[edge_idx[1]], node_idx], dim=-1))
        h_V = self.attention(h_V, h_E, edge_idx)
        h_V = self.update_node(h_V, batch_id)
...
class StructureEncoder(nn.Module):
    ...
    def forward_stage1(self, X, node_idx, edge_idx, batch_id, chain_encoding, V=None, E=None, T_ts=None, batch_id_extend=None, edge_idx_extend=None, virtual_frame_num=0):
        N = batch_id.shape[0]
...
class GeneralE3GNN(nn.Module):
    def __init__(self, gnn_layers, num_hidden, version='CATH-AE'):
        ...
        self.edge_embedding = build_MLP(2, 212, num_hidden, num_hidden)
    ...
    def forward(self, X, node_idx, h_V, batch_id, chain_encoding, topk=30, virtual_frame_num=0):
        N = X.shape[0]
...
class StructDecoder(nn.Module):
    def __init__(self, num_decoder_layers, hidden_dim):
        super().__init__()
        self.embed_condition = nn.Linear(hidden_dim, hidden_dim)
        self.embed_score = nn.Linear(1, hidden_dim)
        self.decgnn = nn.ModuleList([GeneralGNN_noGeo(3,3,3,3, hidden_dim) for i in range(num_decoder_layers-2)])
        self.decse3 = nn.ModuleList([GeneralE3GNN(1, hidden_dim) for i in range(2)])
        


    def forward(self, h_V, node_idx, batch_id, chain_encoding, dec_topk=30):
        X = torch.zeros(h_V.shape[0],4,3, device=h_V.device).float()
        edge_idx = knn_graph(torch.arange(X.shape[0], device=h_V.device).float(), k=dec_topk, batch=batch_id, loop=True, flow='target_to_source')  
        
        for layer in self.decgnn:
            h_V = layer(h_V, node_idx, edge_idx, batch_id)
        
        all_preds = []
        for layer in self.decse3:
            X, _ = layer(X, node_idx, h_V, batch_id, chain_encoding, topk=dec_topk)
            all_preds.append(X)
        return all_preds
