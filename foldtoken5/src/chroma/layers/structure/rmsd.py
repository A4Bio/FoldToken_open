...
from src.chroma.layers.structure import geometry, protein_graph
from torch_scatter import scatter_mean, scatter_sum

class CrossRMSD_Graph(nn.Module):
    ...
    def __init__(self, method="power", method_iter=50, dither=True):
        super(CrossRMSD_Graph, self).__init__()
    ...
    def pairedRMSD(
        ...
        num_batch = scatter_sum(torch.ones_like(batch_id), batch_id, dim=0).size(0)
        num_atoms = X_mobile.size(-3)*X_mobile.size(-2)/num_batch

        # Center coordinates
        X_mobile_mean = scatter_mean(X_mobile, batch_id, dim=0).mean(dim=1, keepdim=True)
        X_target_mean = scatter_mean(X_target, batch_id, dim=0).mean(dim=1, keepdim=True)


        X_mobile_center = X_mobile - X_mobile_mean[batch_id]
        X_target_center = X_target - X_target_mean[batch_id]

        # Cross-covariance matrices contract over atoms
        R = scatter_sum(X_mobile_center[:,:,:,None]*X_target_center[:,:,None,:], batch_id, dim=0).sum(dim=1)
        ...
        #     idx += 1
        #     if idx > 10:
        #         print('eigh is consistently unstable')
        #         break

        # Compute RMSD using top eigenvalue
        norms = scatter_sum((X_mobile_center ** 2).sum(dim=[-1, -2]) + (X_target_center ** 2).sum(dim=[-1, -2]), batch_id, dim=0)
        ...
        if not compute_alignment:
            ...
            X_mobile_transform = torch.einsum("bxr,bir->bix", R[batch_id], X_mobile_center)
            X_mobile_transform = X_mobile_transform + X_target_mean[batch_id]

            rmsd = torch.sum((X_mobile_transform - X_target)**2, dim=-1).mean(-1)
            rmsd_direct = torch.sqrt(scatter_mean(rmsd, batch_id, dim=0))
...
class CrossRMSD(nn.Module):
    ...
    def pairedRMSD(
        ...
        """
        # Collapse all leading batch dimensions
        num_atoms = X_mobile.size(-2) # [8,2000,3]
        batch_dims = list(X_mobile.shape)[:-2]
        X_mobile = X_mobile.reshape([-1, num_atoms, 3])
        X_target = X_target.reshape([-1, num_atoms, 3])
