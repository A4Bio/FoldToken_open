...
ModelBuilder = Callable[[Union[torch.device , str]], nn.Module]


def ESM3_sm_open_v0(device: Union[torch.device , str] = "cpu"):
    model = (
        ESM3(
            d_model=1536,
            n_heads=24,
...
def ESM3_structure_encoder_v0(device: Union[torch.device , str] = "cpu"):
    model = (
        StructureTokenEncoder(
...
def ESM3_structure_decoder_v0(device: Union[torch.device , str] = "cpu"):
    model = (
...
def ESM3_function_decoder_v0(device: Union[torch.device , str] = "cpu"):
...
def load_local_model(model_name: str, device: Union[torch.device , str] = "cpu") -> nn.Module:
