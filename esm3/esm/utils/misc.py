...
def slice_python_object_as_numpy(
    obj: TSequence, idx: Union[int, list[int], slice, np.ndarray]
) -> TSequence:
    """
    ...
    if isinstance(obj, str) and isinstance(sliced_obj, list):
        sliced_obj = "".join(sliced_obj)
    else:
        sliced_obj = obj.__class__(sliced_obj)  # type: ignore
...
def unbinpack(
    tensor: torch.Tensor, sequence_id: Union[torch.Tensor, None], pad_value: Union[int, float]
):
    """
    Args:
        tensor (Tensor): [B, L, ...]
...
def merge_ranges(ranges: list[range], merge_gap_max: Union[int, None]=None) -> list[range]:
...
def maybe_tensor(x, convert_none_to_nan: bool = False) -> Union[torch.Tensor , None]:
    if x is None:
        return None
...
def maybe_list(x, convert_nan_to_none: bool = False) -> Union[list , None]:
    if x is None:
        return None
    x = x.tolist()
